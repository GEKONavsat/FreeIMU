...
final String serialPort = "COM33"; // replace this with your serial port. On windows you will need something like "COM1".
...
float [] EulerUncalib = new float [3]; // psi, theta, phi

int lf = 10; // 10 is '\n' in ASCII
byte[] inBuffer = new byte[22]; // this is the number of chars on each line from the Arduino (including /r/n)
//byte[] inBuffer = new byte[44];

PFont font;
final int VIEW_SIZE_X = 1366, VIEW_SIZE_Y = 740;
...
void readQ() {
  ...
  if(myPort.available() >= 36) {
    String inputString = myPort.readStringUntil('\n');
    //print(inputString);
    if (inputString != null && inputString.length() > 0) {
      String [] inputStringArr = split(inputString, ",");
      //if(inputStringArr.length >= 4) { // q1,q2,q3,q4,\r\n so we have 5 elements
      if(inputStringArr.length >= 8) {
        q[0] = decodeFloat(inputStringArr[0]);
        q[1] = decodeFloat(inputStringArr[1]);
        q[2] = decodeFloat(inputStringArr[2]);
        q[3] = decodeFloat(inputStringArr[3]);
        qU[0] = decodeFloat(inputStringArr[4]);
        qU[1] = decodeFloat(inputStringArr[5]);
        qU[2] = decodeFloat(inputStringArr[6]);
        qU[3] = decodeFloat(inputStringArr[7]);
      }
    }
  }
}
